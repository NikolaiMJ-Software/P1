...
char* PLPR(states* USA) {
    ...
    for (int i = 0; i < STATES; i++) {
        ...
        double tp_votes = USA[i].population - dem_votes - rep_votes;
        ...
        while (total_electors > 0) {
            ...
            if (dem_quotient >= rep_quotient && dem_quotient >= tp_quotient) {
                electors_dem++;
                k_dem += 2; // Increase divisor for Democrats
            } else if (rep_quotient >= dem_quotient && rep_quotient >= tp_quotient) {
                electors_rep++;
                k_rep += 2; // Increase divisor for Republicans
            } else {
                electors_tp++;
                k_tp += 2; // Increase divisor for Third Party
            }
